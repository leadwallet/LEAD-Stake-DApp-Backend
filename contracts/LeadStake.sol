...
contract LeadStake is Ownable {
    ...
    function distribute() external onlyOwner() {
        ...
        if (totalClaimable > IERC20(lead).balanceOf(address(this))) _deposit(totalClaimable.sub(IERC20(lead).balanceOf(address(this))));
       
        emit OnDistribute(msg.sender, now);
    }

    
    function _calculatePayment(address _stakeholder) internal view returns(uint reward){
        
        require(IERC20(lead).balanceOf(address(this)) > 0, 'Empty pool');
        
        uint totalReward = referralRewards[_stakeholder].add(stakeRewards[_stakeholder]).add(stakes[_stakeholder]);
        
        return (totalReward.mul(monthlyROI)).div(100);                                                       //monthly earning
    }
    
    
    function _deposit(uint _amount) internal {
        
        require(IERC20(lead).balanceOf(msg.sender) >= _amount, 'Insufficient LEAD balance in owner wallet');                //owner must have enough LEAD in wallet
    ...
    function withdrawEarnings() public onlyRegistered returns(bool success) {
        ...
        require(stakeRewards[msg.sender] > 0 || referralRewards[msg.sender] > 0, 'No reward to withdraw'); 
